#!/usr/bin/env python3
"""
ground_first 데이터셋으로 공정 비교
Orbit-16 vs PB-NBV-16 (동일 고도, 동일 N, 독립 실행)
"""
import json, math
import numpy as np
...
N_SELECT = 16
MAX_ELLIPSOIDS = 8
ORBIT_ALT = 5.0   # 물체 위 5m
ORBIT_RAD = 15.0
SEED = 0
np.random.seed(SEED)

OUT_DIR = Path("results/ground_first_comparison")
...
def evaluate(cam_pos, occ_ell, fro_ell):
    ...
    for r,(z,a,is_f) in enumerate(items):
        F+=(a*0.5**r) if is_f else -(a*0.5**r)
    return F

def make_candidates():
    cands=[]
    z = TARGET[2] - ORBIT_ALT
    for rad in [ORBIT_RAD, ORBIT_RAD+3]:
        n_az=max(12, int(2*math.pi*rad/2.5))
        for i in range(n_az):
            th=2*math.pi*i/n_az
            cands.append([TARGET[0]+rad*math.cos(th), TARGET[1]+rad*math.sin(th), z])
    return np.array(cands)

def az_stats(positions):
    angles=sorted([math.degrees(math.atan2(p[1]-TARGET[1],p[0]-TARGET[0]))%360 for p in positions])
    gaps=[(angles[(i+1)%len(angles)]-angles[i])%360 for i in range(len(angles))]
    return angles, gaps, max(gaps), np.mean(gaps), np.std(gaps)

# ═══════════════════════════════════════════
print("\n" + "="*55)
print("Orbit-16 (고도 고정 5m)")
print("="*55)
observed_o = np.zeros(N_SURF, dtype=bool)
orbit_path = []
for i in range(N_SELECT):
    angle = 2*math.pi*i/N_SELECT
    pos = np.array([TARGET[0]+ORBIT_RAD*math.cos(angle),
                    TARGET[1]+ORBIT_RAD*math.sin(angle),
                    TARGET[2]-ORBIT_ALT])
    obs=observed_by(pos); gained=(~observed_o[obs]).sum()
    observed_o[obs]=True; cov=observed_o.sum()/N_SURF
    ...
    print(f"  step{i+1:2d}: az={az:6.1f}° +{gained:4d}voxel cov={cov*100:5.1f}%")

az_o,gaps_o,max_o,mean_o,std_o = az_stats([p['pos'] for p in orbit_path])
print(f"\n  Max gap: {max_o:.1f}° | Std: {std_o:.1f}° | Coverage: {orbit_path[-1]['coverage']*100:.1f}%")

# ═══════════════════════════════════════════
print("\n" + "="*55)
print("PB-NBV-16 (고도 고정 5m, 독립 실행)")
print("="*55)
...
for step in range(N_SELECT):
    ...
    best=int(np.argmax(scores)); used[best]=True
    chosen=cands[best]
    obs=observed_by(chosen); gained=(~observed_n[obs]).sum()
    observed_n[obs]=True; cov=observed_n.sum()/N_SURF
    ...
    nbv_path.append({'step':step+1,'pos':chosen.tolist(),'azimuth':round(az,1),
                     'gained':int(gained),'coverage':round(cov,4),
                     'score':round(float(scores[best]),1)})
    print(f"  step{step+1:2d}: az={az:6.1f}° F={scores[best]:9.1f} +{gained:4d}voxel cov={cov*100:5.1f}%")
    if cov>0.999: print("  (완전 커버)"); break

az_n,gaps_n,max_n,mean_n,std_n = az_stats([p['pos'] for p in nbv_path])
print(f"\n  Max gap: {max_n:.1f}° | Std: {std_n:.1f}° | Coverage: {nbv_path[-1]['coverage']*100:.1f}%")

# ═══════════════════════════════════════════
print("\n" + "="*55)
print("비교 요약")
print("="*55)
print(f"{'':20} {'Orbit-16':>14} {'PB-NBV-16':>14}")
print(f"{'Max gap':20} {max_o:>13.1f}° {max_n:>13.1f}°")
print(f"{'Std gap':20} {std_o:>13.1f}° {std_n:>13.1f}°")
print(f"{'Coverage':20} {orbit_path[-1]['coverage']*100:>13.1f}% {nbv_path[-1]['coverage']*100:>13.1f}%")

result = {'orbit':{'max_gap':round(max_o,2),'std_gap':round(std_o,2),
                   'coverage':orbit_path[-1]['coverage'],
                   'azimuths':[round(a,1) for a in az_o],
                   'gaps':[round(g,1) for g in gaps_o], 'path':orbit_path},
          'pbnbv':{'max_gap':round(max_n,2),'std_gap':round(std_n,2),
                   'coverage':nbv_path[-1]['coverage'],
                   'azimuths':[round(a,1) for a in az_n],
                   'gaps':[round(g,1) for g in gaps_n], 'path':nbv_path}}

with open(OUT_DIR/'result.json','w') as f:
    json.dump(result,f,indent=2)
print(f"\n✓ 저장: {OUT_DIR/'result.json'}")
